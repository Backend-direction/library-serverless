...
const Feedback: FunctionComponent<FeedbackProps> = ({ feedback }) => {
  return (
    <Box mt={2}>
      <Wrapper elevation={0}>
        <Box sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', marginBottom: '10px' }}>
          <Box sx={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <Typography variant='caption' color='#5B636F'>{feedback.date}</Typography>
          </Box>
          <Box sx={{ display: 'flex', flexDirection: 'column', textAlign: 'end' }}>
            <Rating name="read-only" value={feedback.rate} readOnly size="small" />
            <Typography component="legend" variant='caption'>{feedback.rate} star(s)</Typography>
          </Box>
        </Box>
        <Box>
        ...
        <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent:' space-between' }}>
          <Link href="#" underline="none">
            <Typography variant='subtitle2' color="#5B636F">Read whole book</Typography>
          </Link>
          <Box>
